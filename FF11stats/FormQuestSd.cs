...
namespace FF11stats
{
    public partial class FormQuestSd : Form
    {
        public FormQuestSd()
        {
            InitializeComponent();
            ...
            foreach( Control c in flowLayoutPanel1.Controls ) {
                if( c is CheckBox b ) {
                    if( cd.QuestSand[int.Parse( b.Name.Substring( 2 ) )] != 0 ) {
                        l++;
                        b.Checked = true;
                    }
                }
        ...
        private void Apply()
        {
            foreach( Control c in flowLayoutPanel1.Controls ) {
                if( c is CheckBox b ) {
                    cd.QuestSand[int.Parse( b.Name.Substring( 2 ) )] = Convert.ToInt16( b.Checked );
                }
            }
            return;
        }
        ...
        private void Sd074_CheckedChanged( object sender, EventArgs e )
        {
            if( sd074.Checked is true ) {
                sd075.Checked = false;
                sd075.Enabled = false;
            } else {
                sd075.Enabled = true;
            }
            if( sd075.Checked is true ) {
                sd074.Checked = false;
                sd074.Enabled = false;
            } else {
                sd074.Enabled = true;
