...
namespace FF11stats
{
    public partial class FormQuestOt : Form
    {
        public FormQuestOt()
        {
            InitializeComponent();
            ...
            foreach( Control c in flowLayoutPanel1.Controls ) {
                if( c is CheckBox b ) {
                    if( cd.QuestOthers[int.Parse( b.Name.Substring( 2 ) )] != 0 ) {
                        l++;
                        b.Checked = true;
                    }
                }
        ...
        private void Apply()
        {
            foreach( Control c in flowLayoutPanel1.Controls ) {
                if( c is CheckBox b ) {
                    cd.QuestOthers[int.Parse( b.Name.Substring( 2 ) )] = Convert.ToSByte( b.Checked );
                }
            }
            return;
        }
        ...
        private void Ot010_CheckedChanged( object sender, EventArgs e )
        {
            if( ot010.Checked is true ) {
                ot026.Checked = false;
                ot026.Enabled = false;
            } else {
                ot026.Enabled = true;
            }
            if( ot026.Checked is true ) {
                ot010.Checked = false;
                ot010.Enabled = false;
            } else {
                ot010.Enabled = true;
